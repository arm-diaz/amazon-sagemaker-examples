"""
SageMaker Pipeline for Fuel Delivery Optimization
====================================================
...
class FuelDeliveryPipeline:
    """End-to-end pipeline for fuel delivery forecasting with SageMaker and AutoGluon."""
    ...
    def __init__(
        ...
        prefix: str = "fuel-delivery-optimization",
    ...
    def train(
        ...
        if not src_dir:
            raise ValueError(
                ...
                "Example: pipeline = FuelDeliveryPipeline(source_dir='../src')"
            )
        ...
        hp = {
            ...
            "target-column": "daily_consumption",
            "item-id-column": "tank_id",
            "timestamp-column": "reading_date",
        }
        ...
        training_image = self._get_container_image(
            scope="training", instance_type=instance_type
        )

        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        job_name = f"fuel-delivery-forecast-{timestamp}"
        ...
        inputs = {
            "train": sagemaker.inputs.TrainingInput(
                data_uris["train"], content_type="text/csv"
            )
        }
        if "validation" in data_uris:
            inputs["validation"] = sagemaker.inputs.TrainingInput(
                data_uris["validation"], content_type="text/csv"
            )
    ...
    def create_model(
        ...
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        model_name = model_name or f"fuel-delivery-model-{timestamp}"

        inference_image = self._get_container_image(
            scope="inference", instance_type=instance_type
        )
    ...
    def deploy(
        ...
        wait: bool = True,
        source_dir: Optional[str] = None,
    ) -> "FuelDeliveryPredictor":
        model_name = model_name or self.model_name
        if not model_name:
            model_name = self.create_model(
                instance_type=instance_type, source_dir=source_dir
            )

        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        endpoint_name = endpoint_name or f"fuel-delivery-endpoint-{timestamp}"
    ...
    def batch_transform(
        ...
        job_name = f"fuel-delivery-batch-{timestamp}"
    ...
    def cleanup(self, delete_endpoint: bool = True, delete_model: bool = True):
